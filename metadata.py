...
class Butterfly():
    def __init__(self, row):
        ...
        self.higher_classification = row[3]
        self.sex = row[4]
        self.latitude = row[5]
        ...
        self.year = row[8]
        self.id = int(row[9])
        self.occurence_id = row[10]
        self.image_id = row[11]

    @property
    def image_filename(self):
        return 'data/raw/{!s}.jpg'.format(self.id)
...
def load():
    ...
    row_index = 0
    progress = tqdm(total=8435, desc='Reading metadata.csv')
